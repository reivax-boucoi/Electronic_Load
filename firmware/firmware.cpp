...
float fan_temp = 30.0; //fan temp
float outOfRegOn=1.0; // out of reg enable/disable
float eff = 0;
...
Value setting3V(&outOfRegOn,15,0,1,0,1);
...
float calc_iload(void) {
    ...
    /*if (load.onState && outOfRegOn>0.5 && ((res * (1.0 + OUTOFREGPERCENT)) < idisp_load || (res * (1.0 - OUTOFREGPERCENT)) > idisp_load) ) {
...
void build_menu(void) {
    ...
    settingsScreen.txt1 = F("Batt     V Reg  ");
    ...
    settingsScreen.addValue(&setting3V);
